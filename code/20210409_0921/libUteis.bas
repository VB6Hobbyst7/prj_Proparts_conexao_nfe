...
'' COMPRAS
'' -- AJUSTE DE CAMPOS
Private Const qryUpdateCurrecy As String = _
        "UPDATE tblProcessamento SET tblProcessamento.valor = Format(Replace([tblProcessamento].[valor], '.', ','), '#,##0.00') WHERE (((tblProcessamento.NomeCampo)='strCampo'));"

Private Const qryUpdateDate As String = _
        "UPDATE tblProcessamento SET tblProcessamento.valor = CDate(Replace(Mid([tblProcessamento].[valor],1,10),'-','/')) WHERE (((tblProcessamento.NomeCampo)='strCampo'));"

Private Const qryUpdateTime As String = _
        "UPDATE tblProcessamento SET tblProcessamento.valor = Replace(Mid([tblProcessamento].[valor],12,8),'-','/') WHERE (((tblProcessamento.NomeCampo)='strCampo'));"
...
'' VERIFICAR A EXISTENCIA DE CONSULTAS
