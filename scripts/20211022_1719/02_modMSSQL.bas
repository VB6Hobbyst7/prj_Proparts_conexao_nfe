...
Function carregarScript_Insert() As String
...
Dim strCamposNomes As String: strCamposNomes = carregarCamposNomes(pRepositorio)
Dim strCamposNomesTmp As String: strCamposNomesTmp = Replace(carregarCamposNomes(pRepositorio), "_CompraNF", "")
Dim strCamposValores As String: strCamposValores = carregarCamposValores(pRepositorio, pChvAcesso)

Dim tmpScript As String: tmpScript = "INSERT INTO tblCompraNF ( strCamposNomes ) SELECT strCamposNomesTmp FROM ( VALUES ( strCamposValores ) ) AS TMP ( strCamposNomesTmp ) LEFT JOIN tblCompraNF ON tblCompraNF.ChvAcesso_CompraNF = tmp.ChvAcesso WHERE tblCompraNF.ChvAcesso_CompraNF IS NULL;"

carregarScript_Insert = _
    Replace(Replace(Replace(tmpScript, "strCamposNomes", strCamposNomes), "strCamposNomesTmp", strCamposNomesTmp), "strCamposValores", strCamposValores)
