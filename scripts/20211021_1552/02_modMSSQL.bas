...
Dim sqlScript As String: sqlScript = "insert into tblCompraNF (ChvAcesso_CompraNF) select ChvAcesso from ( values sqlItens ) as tmp(ChvAcesso) left join tblCompraNF on tblCompraNF.ChvAcesso_CompraNF = tmp.ChvAcesso  where tblCompraNF.ChvAcesso_CompraNF is null;"
...
'dbDestino.SqlSelect "SELECT * FROM tblCompraNF"
    ...
    Do While Not rstChvAcesso.EOF
        ...
        sqlItens = sqlItens & "('" & rstChvAcesso.Fields("ChvAcesso").value & "')"
        dbDestino.SqlExecute Replace(sqlScript, "sqlItens", sqlItens)
        
        rstChvAcesso.MoveNext
        sqlItens = ""
    Loop

'    sqlItens = left(sqlItens, Len(sqlItens) - 1)
'    TextFile_Append CurrentProject.path & "\" & strLog(), sqlItens
