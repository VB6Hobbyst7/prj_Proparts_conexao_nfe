...
If DLookup("[ValorDoParametro]", "[tblParametros]", "[TipoDeParametro]='debug'") Then TextFile_Append CurrentProject.path & "\" & strLog(), sqlCampos
...
'    Debug.Print tmpScript
    ...
    If DLookup("[ValorDoParametro]", "[tblParametros]", "[TipoDeParametro]='debug'") Then TextFile_Append CurrentProject.path & "\" & strLog(), tmpSelecaoDeRegistros
    ...
    Do While Not rstOrigem.EOF
        ...
        If DLookup("[ValorDoParametro]", "[tblParametros]", "[TipoDeParametro]='debug'") Then TextFile_Append CurrentProject.path & "\" & strLog(), tmpScript & tmp
        ...
        If (pDestino = "tblCompraNFItem") Then
            ...
            Debug.Print tmp
            If DLookup("[ValorDoParametro]", "[tblParametros]", "[TipoDeParametro]='debug'") Then TextFile_Append CurrentProject.path & "\" & strLog(), tmp
...
''' Relacionar IdCompra Com ChvAcesso
'Public Function relacionarIdCompraComChvAcesso()
'
''' #BANCO_LOCAL - PARAMETROS DE ACESSO
'Dim dbLocal As DAO.Database: Set dbLocal = CurrentDb
'Dim rstLocal As DAO.Recordset
'
''' #BANCO_SERVIDOR - PARAMETROS DE ACESSO
'Dim dbServidor As New Banco
'Dim strUsuarioNome As String: strUsuarioNome = DLookup("[ValorDoParametro]", "[tblParametros]", "[TipoDeParametro]='BancoDados_Usuario'")
'Dim strUsuarioSenha As String: strUsuarioSenha = DLookup("[ValorDoParametro]", "[tblParametros]", "[TipoDeParametro]='BancoDados_Senha'")
'Dim strOrigem As String: strOrigem = DLookup("[ValorDoParametro]", "[tblParametros]", "[TipoDeParametro]='BancoDados_Origem'")
'Dim strBanco As String: strBanco = DLookup("[ValorDoParametro]", "[tblParametros]", "[TipoDeParametro]='BancoDados_Banco'")
'
''' #REPOSITORIO_LOCAL
'Dim Script As New clsConexaoNfeCte
'
'    '' #REPOSITORIO_LOCAL - RELACIONAR ID_COMPRA COM CHV_ACESSO
'    Script.criarTabelaTemporariaParaRelacionarIdCompraComChvAcesso
'    Set rstLocal = dbLocal.OpenRecordset(Script.SelecRepositorioDeRelacao)
'
'    '' #BANCO_SERVIDOR - CONEXÃO
'    dbServidor.Start strUsuarioNome, strUsuarioSenha, strOrigem, strBanco, drSqlServer
'    dbServidor.SqlSelect Script.SelecRepositorioDeCompras
'
'    '' #BANCO_LOCAL - CADASTRO DE COMPRAS
'    Do While Not dbServidor.rs.EOF
'        rstLocal.AddNew
'        rstLocal.Fields("ID_CompraNF").value = dbServidor.rs.Fields("ID_CompraNF").value
'        rstLocal.Fields("ChvAcesso_CompraNF").value = dbServidor.rs.Fields("ChvAcesso_CompraNF").value
'        rstLocal.Fields("NumPed_CompraNF").value = dbServidor.rs.Fields("NumPed_CompraNF").value
'        rstLocal.Update
'
'        dbServidor.rs.MoveNext
'        DoEvents
'    Loop
'
'    '' #CHAVES_DE_ACESSO - RELACIONAR ITENS DE COMPRAS COM CHAVES DE ACESSO VINDAS DO SERVIDOR
'    Application.CurrentDb.Execute "UPDATE tmpCompras_ID_CompraNF " & _
'                                    "INNER JOIN tblCompraNFItem ON tmpCompras_ID_CompraNF.ChvAcesso_CompraNF = tblCompraNFItem.ChvAcesso_CompraNF " & _
'                                    "SET tblCompraNFItem.ID_CompraNF_CompraNFItem = [tmpCompras_ID_CompraNF].[ID_CompraNF]; "
'
'dbServidor.CloseConnection
'rstLocal.Close
'Set Script = Nothing
'
'End Function
