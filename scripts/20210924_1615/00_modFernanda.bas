...
Sub moverArquivo()
Dim strOrigem As String: strOrigem = "C:\temp\coleta\68.365.5010001-05 - Proparts Comércio de Artigos Esportivos e Tecnologia Ltda\32210304884082000569570000040073831040073834-cteproc.xml"
Dim strDestino As String: strDestino = "C:\temp\Processados\"
Dim objFSO As Object: Set objFSO = CreateObject("Scripting.FileSystemObject")

'' 01. CRIAR PASTA DESTINO
CreateDir Replace(strOrigem, getPath(strOrigem), strDestino)

'' 02. MOVER ARQUIVO ORIGINAL PARA A PASTA DESTINO
objFSO.CopyFile strOrigem, strDestino & getFileNameAndExt(rstArquivos.Fields("CaminhoDoArquivo").value)
...
'' CAMINHO PARA ARQUIVOS PROCESSADOS
Dim strProcessados As String: strProcessados = DLookup("[ValorDoParametro]", "[tblParametros]", "[TipoDeParametro]='caminhoDeProcessados'"): CreateDir strProcessados

Dim strExpurgo As String: strExpurgo = DLookup("[ValorDoParametro]", "[tblParametros]", "[TipoDeParametro]='caminhoDeExpurgo'"): CreateDir strExpurgo

Dim sqlArquivos As String: sqlArquivos = _
...
Debug.Print sqlArquivos

Dim qryUpdateRegistroProcessado As String: qryUpdateRegistroProcessado = _
        "UPDATE tblDadosConexaoNFeCTe SET tblDadosConexaoNFeCTe.registroProcessado = 3 WHERE (((tblDadosConexaoNFeCTe.ChvAcesso)=""strChave"") AND ((tblDadosConexaoNFeCTe.registroValido)=1) AND ((tblDadosConexaoNFeCTe.registroProcessado)=2));"

Debug.Print qryUpdateRegistroProcessado
...
Do While Not rstArquivos.EOF
    ...
    objFSO.CopyFile strFileName, strProcessados & getFileNameAndExt(rstArquivos.Fields("CaminhoDoArquivo").value)
    'If (Dir(strFileName) <> "") Then Kill strProcessados & strFileName
    
    qryTemp = Replace(qryUpdateRegistroProcessado, "strChave", rstArquivos.Fields("ChvAcesso").value)
    Debug.Print qryTemp
