...
totalDeRegistros = arquivos.count
    ...
    For Each item In arquivos
        ...
        If DLookup("[ValorDoParametro]", "[tblParametros]", "[TipoDeParametro]='debug'") Then TextFile_Append CurrentProject.path & "\" & strLog(), "carregarDadosGerais " & contadorDeRegistros & " - " & CStr(totalDeRegistros)
...
arquivos.add "C:\xmls\68.365.5010001-05 - Proparts Comércio de Artigos Esportivos e Tecnologia Ltda\recebimento\32210304884082000569570000040073831040073834-cteproc.xml"
...
adm_Err:
    ...
    If DLookup("[ValorDoParametro]", "[tblParametros]", "[TipoDeParametro]='debug'") Then TextFile_Append CurrentProject.path & "\" & strLog(), "processarArquivosPendentes() - " & Err.Description
    Resume adm_Exit
...
Dim DT_PROCESSO As Date: DT_PROCESSO = Now()
    ...
    If DLookup("[ValorDoParametro]", "[tblParametros]", "[TipoDeParametro]='debug'") Then TextFile_Append CurrentProject.path & "\" & strLog(), "Concluido! - enviarDadosServidor"
...
Dim qrySelectRegistroValido As String: qrySelectRegistroValido = _
    ...
    If DLookup("[ValorDoParametro]", "[tblParametros]", "[TipoDeParametro]='debug'") Then TextFile_Append CurrentProject.path & "\" & strLog(), qrySelectRegistroValido
...
'    s.criarArquivoJson opManifesto, qrySelectRegistroValido, strCaminhoDeSaida
    ...
    If DLookup("[ValorDoParametro]", "[tblParametros]", "[TipoDeParametro]='debug'") Then TextFile_Append CurrentProject.path & "\" & strLog(), "Concluido! - criacaoArquivosJson"
