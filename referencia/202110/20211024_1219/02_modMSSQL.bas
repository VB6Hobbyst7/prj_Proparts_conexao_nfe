...
Dim rstChvAcesso As DAO.Recordset: Set rstChvAcesso = db.OpenRecordset(Scripts.SelectRegistroValidoPorcessado)
    ...
    dbDestino.SqlSelect "SELECT ChvAcesso_CompraNF,ID_CompraNF FROM tblCompraNF"
    
    
    'dbDestino.rs.index = "ChvAcesso_CompraNF"
    
    
    dbDestino.rs.index = "ChvAcesso_CompraNF"
    
    Do While Not rstChvAcesso.EOF
        'dbDestino.rs.Find ("ChvAcesso_CompraNF = " & rstChvAcesso.Fields("ChvAcesso_CompraNF").value)
        dbDestino.rs.Seek rstChvAcesso.Fields("ChvAcesso_CompraNF").value, adSeekFirstEQ
        
        If Not dbDestino.rs.EOF Then Debug.Print dbDestino.rs.Fields("ID_CompraNF").value
